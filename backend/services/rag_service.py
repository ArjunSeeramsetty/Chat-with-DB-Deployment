...
from backend.core.types import (
    QueryRequest, QueryResponse, QueryAnalysis, ContextInfo, 
    SQLGenerationResult, ExecutionResult, VisualizationRecommendation,
    ProcessingMode, QueryType, IntentType
)
...
from backend.core.schema_linker import SchemaLinker, SchemaContext
from backend.core.candidate_ranker import CandidateRanker, RankedCandidate
from backend.core.executor import SQLExecutor, AsyncSQLExecutor
...
class EnhancedRAGService:
    ...
    def __init__(self, db_path: str, llm_provider=None, memory_service=None):
        ...
        self.enhanced_validator = EnhancedSQLValidator(self.schema_info, llm_provider=self.llm_provider)
        self.sql_validator = LegacySQLValidator(self.schema_info)  # Legacy validator for backward compatibility
        self.schema_linker = SchemaLinker(self.schema_info, db_path, llm_provider=self.llm_provider)
    ...
    async def process_query(self, request: QueryRequest) -> QueryResponse:
        ...
        try:
            logger.info(f"Starting query processing - Question: {request.question[:100]} - User: {request.user_id} - Mode: {request.processing_mode.value} - Clarification attempts: {request.clarification_attempt_count}")
            ...
            if request.clarification_answers and len(request.clarification_answers) > 0:
                logger.info(f"Processing query with clarification answers: {request.clarification_answers}")
                # Enhance the question with clarification context
                enhanced_question = self._enhance_question_with_clarification(request.question, request.clarification_answers)
            ...
            query_analysis = await self.intent_analyzer.analyze_intent(rewritten_question, self.llm_provider)
            ...
            logger.info(f"Re-extracted entities from original query: {original_entities}")
            # Add original query for manual entity extraction
            query_analysis._original_query = request.question
            logger.info(f"Intent analysis completed - Type: {query_analysis.query_type.value}, Intent: {query_analysis.intent.value}, Confidence: {query_analysis.confidence}")
            
            # Step 2: Build Context
            context_info = self._build_context(rewritten_question, query_analysis, request.user_id)
            logger.info(f"Context built - User mappings: {len(context_info.user_mappings)}, Dimension values: {len(context_info.dimension_values)}")
            ...
            if hasattr(sql_result, 'clarification_question') and sql_result.clarification_question:
                attempt_count = request.clarification_attempt_count + 1
                logger.info(f"Generating clarification response. Current attempt: {request.clarification_attempt_count}, New attempt: {attempt_count}")
                response = QueryResponse(
                    success=False,
                ...
                logger.info(f"Generated clarification response with attempt count: {response.clarification_attempt_count}")
                logger.info(f"Response object: {response}")
                return response
            
            # === LLM Hook #6 – "Contextual Clarifier" ===
            if not sql_result.success or (sql_result.confidence is not None and sql_result.confidence < 0.6):
                if self.llm_provider:
                    try:
                        # Build contextual clarification prompt
                        detected_info = []
                        if query_analysis.entities:
                            detected_info.append(f"Detected entities: {', '.join(query_analysis.entities)}")
                        if query_analysis.time_period:
                            detected_info.append(f"Detected time period: {query_analysis.time_period}")
                        if query_analysis.query_type:
                            detected_info.append(f"Query type: {query_analysis.query_type.value}")
                        
                        detected_context = "\n".join(detected_info) if detected_info else "No specific entities or time periods detected."
...
Generate ONLY the clarification question, no explanations or additional text:"""
                        ...
                        if llm_response and hasattr(llm_response, 'content') and llm_response.content:
                            ...
                            clarification_question = self._clean_clarification_question(clarification_question)
                            logger.info(f"LLM Hook #6: Generated clarification question: {clarification_question}")
...
Generate ONLY the clarification question, no explanations or additional text:"""
            ...
            try:
                ...
                logger.info(f"Sync SQL execution result: success={execution_result.success}, row_count={execution_result.row_count}, error={execution_result.error}")
            except Exception as e:
            ...
            logger.info(f"SQL execution result: success={execution_result.success}, row_count={execution_result.row_count}, error={execution_result.error}")
            ...
            if not execution_result.success:
                ...
                response = QueryResponse(
                    ...
                    correlation_id=correlation_id or str(uuid.uuid4()),  # Provide default if None
                    processing_time=time.time() - start_time,
                    processing_mode=request.processing_mode.value,
                    query_type=query_analysis.query_type.value,
                    api_calls=self._count_api_calls(request.processing_mode, None)
            ...
            if execution_result.row_count > 0:
                visualization = self._generate_visualization(execution_result.data, request.question)
            
            # Step 6: Generate Suggestions
            suggestions = self._generate_suggestions(request.question, execution_result.data, query_analysis)
            
            # === LLM Hook #7 – "NL Result Narration" ===
            summary = "Query executed successfully."  # Default summary
            if execution_result.success and execution_result.row_count > 0 and self.llm_provider:
                try:
            ...
            response = QueryResponse(
                ...
                summary=summary,
                llm_model=getattr(self.llm_provider, 'model', None) if self.llm_provider else None,
                gpu_status=self._get_gpu_status(),
                confidence=sql_result.confidence,  # Include confidence score
                clarification_attempt_count=request.clarification_attempt_count
            )
            
            logger.info(f"Query processing completed successfully - Rows: {execution_result.row_count}, Time: {response.processing_time:.3f}s")
            return response
    ...
    def _build_context(self, query: str, query_analysis: QueryAnalysis, user_id: str) -> ContextInfo:
        ...
        if self.memory_service:
            try:
                memory_context = self.memory_service.get_relevant_context(query, user_id)
            except Exception as e:
    ...
    async def _generate_sql_or_request_clarification(self, query: str, analysis: QueryAnalysis, request: QueryRequest) -> SQLGenerationResult:
        """
        ...
        try:
            ...
            if is_trend_analysis:
                logger.info(f"Trend analysis query detected, using dynamic template-based generation")
                print(f"🔍 TREND ANALYSIS DETECTED, using dynamic template-based generation")
            ...
            if any(pattern in query_lower for pattern in clear_patterns):
                logger.info(f"Clear query pattern detected, trying direct SQL generation")
            ...
            if confidence >= 0.4:
                print(f"🔍 HIGH CONFIDENCE ({confidence:.3f}), trying direct SQL generation")
                logger.info(f"High confidence ({confidence:.3f}), trying direct SQL generation")
                direct_sql = self._generate_direct_sql(query, analysis)
                if direct_sql:
            ...
            if hasattr(request, 'clarification_answers') and request.clarification_answers:
                enhanced_query = self._enhance_question_with_clarification(query, request.clarification_answers)
...
Generate ONLY the clarification question, no explanations or additional text:"""
    ...
    async def _generate_llm_sql(self, query: str, query_analysis: QueryAnalysis, 
                         enhanced_context: str) -> Optional[str]:
...
"""
    ...
    def _get_energy_column_for_table(self, table_name: str) -> str:
        ...
        for col in schema_info[table_name].get('columns', []):
            col_lower = col.lower()
            if any(term in col_lower for term in ['energy', 'generation', 'demand', 'met']):
    ...
    def _generate_visualization(self, data: List[Dict[str, Any]], query: str) -> Optional[VisualizationRecommendation]:
        ...
        try:
            ...
            logger.info(f"🔍 VISUALIZATION DEBUG - Sample data: {data[0] if data else 'No data'}")
            ...
            for header in headers:
                ...
                numeric_keywords = ['value', 'amount', 'total', 'sum', 'count', 'percentage', 'growth', 
                                 'current', 'previous', 'generation', 'consumption', 'shortage', 'met',
                                 'maximum', 'minimum', 'average', 'avg', 'max', 'min']
                non_numeric_keywords = ['name', 'region', 'state', 'source', 'trend', 'month', 'year', 
                                      'quarter', 'week', 'day', 'id', 'type', 'category', 'description', 'status']
            ...
            if is_monthly_multi_state or has_growth_data:
                ...
                for header in headers:
                    header_lower = header.lower()
                    if (not any(num_word in header_lower for num_word in ['month', 'growth', 'current', 'previous', 'percentage']) and
                        not any(numeric_word in header_lower for numeric_word in numeric_keywords)):
    ...
    def _generate_suggestions(self, query: str, data: List[Dict[str, Any]], 
    ...
    def _format_table_data(self, execution_result: ExecutionResult) -> Dict[str, Any]:
        ...
        rows = [[row.get(header, '') for header in headers] for row in execution_result.data]
    ...
    def _count_api_calls(self, processing_mode: ProcessingMode, visualization: Optional[VisualizationRecommendation]) -> int:
    ...
    def _map_user_references(self, query: str, dimension_values: Dict[str, List[Any]]) -> List[Any]:
        """Map user references to dimension values"""
    ...
    async def _get_schema_info_async(self) -> Optional[Any]:
        ...
        if (self._schema_cache is None or 
            current_time - self._schema_cache_time > self._cache_ttl):
            
            try:
                # Sprint 4: Use async schema info retrieval for better performance
                self._schema_cache = await self.async_sql_executor.get_schema_info_async()
    ...
    def _get_gpu_status(self) -> Dict[str, Any]:
        ...
        return {
            "gpu_enabled": False,
            "gpu_used": False,
            "model": getattr(self.llm_provider, 'model', 'unknown') if self.llm_provider else 'unknown'
        }
    ...
    def _generate_schema_based_clarification(self, query: str, analysis: QueryAnalysis) -> str:
        """
        ...
        if "maximum" in query_lower and ("energy" in query_lower or "consumption" in query_lower):
    ...
    def _enhance_question_with_clarification(self, original_question: str, clarification_answers: Dict[str, str]) -> str:
        """
        ...
        for question, answer in clarification_answers.items():
            ...
            if "energyshortage" in answer_lower or "energy not supplied" in answer_lower or "energy shortage" in answer_lower:
                enhancement_parts.append("EnergyShortage (energy not supplied)")
            elif "energymet" in answer_lower or "energy actually supplied" in answer_lower or "energy supplied" in answer_lower or "energy met" in answer_lower:
            ...
            elif "energyoutage" in answer_lower or "energy outage" in answer_lower or "total outage" in answer_lower:
                enhancement_parts.append("EnergyOutage (total energy interrupted)")
            elif "energyrestoration" in answer_lower or "energy restoration" in answer_lower or "restoration" in answer_lower:
                enhancement_parts.append("EnergyRestoration (energy restoration completed)")
            elif "central sector outage" in answer_lower or "central sector" in answer_lower:
                enhancement_parts.append("CentralSectorOutage")
            elif "state sector outage" in answer_lower or "state sector" in answer_lower:
                enhancement_parts.append("StateSectorOutage")
            elif "private sector outage" in answer_lower or "private sector" in answer_lower:
            ...
            elif "yearly" in answer_lower or "annual" in answer_lower or "entire year" in answer_lower:
        ...
        if enhancement_parts:
            ...
            enhanced_question = f"{original_question} [Clarified: {'; '.join(enhancement_parts)}]"
    ...
    def _build_contextual_clarification_prompt(self, query: str, analysis: QueryAnalysis, 
                                             clarification_answers: Dict[str, str] = None) -> str:
        """
        ...
        query_lower = query.lower()
        context_specific_examples = self._get_context_specific_examples(query_lower, analysis)
...
Generate ONLY the clarification question, no explanations or additional text:"""
    ...
    def _is_similar_question(self, question1: str, question2: str) -> bool:
        ...
        q1_clean = q1_lower.replace("do you want", "").replace("for all states", "").replace("for all regions", "").replace("for 2025", "").strip()
        q2_clean = q2_lower.replace("do you want", "").replace("for all states", "").replace("for all regions", "").replace("for 2025", "").strip()
        ...
        if len(union) > 0:
            ...
            logger.info(f"Similarity score: {similarity:.2f} between '{q1_clean}' and '{q2_clean}'")
    ...
    def _get_geographic_level_for_query(self, query_lower: str) -> str:
        ...
        elif "demand" in query_lower or "energy" in query_lower or "shortage" in query_lower:
    ...
    def _get_context_specific_examples(self, query_lower: str, analysis: QueryAnalysis) -> str:
        """
        ...
        if "outage" in query_lower:
            ...
            examples.append(f"- 'Do you want CentralSectorOutage, StateSectorOutage, or PrivateSectorOutage for all {geographic_level} in 2025?'")
            examples.append(f"- 'For outage data, do you want CentralSectorOutage, StateSectorOutage, or PrivateSectorOutage for all {geographic_level}?'")
            examples.append(f"- 'Do you want the total outage duration or the number of outage incidents for all {geographic_level}?'")
        
        elif "maximum" in query_lower and ("energy" in query_lower or "consumption" in query_lower):
            examples.append("Examples for MAXIMUM ENERGY CONSUMPTION queries:")
            examples.append(f"- 'Do you want EnergyMet (energy actually supplied) or MaximumDemand (peak demand capacity) for all {geographic_level} in 2025?'")
            examples.append(f"- 'For maximum energy consumption, do you want the maximum EnergyMet value or the maximum DemandMet value for all {geographic_level}?'")
            examples.append(f"- 'Do you want the state with the highest EnergyMet or the state with the highest MaximumDemand for all {geographic_level}?'")
        
        elif "demand" in query_lower:
            examples.append("Examples for DEMAND queries:")
            examples.append(f"- 'Do you want DemandMet (actual demand met) or MaximumDemand (peak demand capacity) for all {geographic_level} in 2025?'")
            examples.append(f"- 'For demand data, do you want PeakDemand, AverageDemand, or TotalDemand for all {geographic_level}?'")
            examples.append(f"- 'Do you want the maximum demand value or the average demand value for all {geographic_level}?'")
        
        elif "shortage" in query_lower:
            examples.append("Examples for SHORTAGE queries:")
            examples.append(f"- 'Do you want EnergyShortage (energy not supplied) or PowerShortage (power not supplied) for all {geographic_level} in 2025?'")
            examples.append(f"- 'For shortage data, do you want TotalShortage, PeakShortage, or AverageShortage for all {geographic_level}?'")
            examples.append(f"- 'Do you want the total shortage amount or the shortage percentage for all {geographic_level}?'")
        
        elif "generation" in query_lower:
            examples.append("Examples for GENERATION queries:")
            examples.append(f"- 'Do you want TotalGeneration, ThermalGeneration, or RenewableGeneration for all {geographic_level} in 2025?'")
            examples.append(f"- 'For generation data, do you want CoalGeneration, GasGeneration, or HydroGeneration for all {geographic_level}?'")
            examples.append(f"- 'Do you want the total generation capacity or the actual generation output for all {geographic_level}?'")
        
        else:
            # Generic examples for other query types
            examples.append("Examples for general queries:")
            examples.append(f"- 'Do you want EnergyShortage (energy not supplied) or EnergyMet (energy actually supplied) for all {geographic_level} in 2025?'")
            examples.append(f"- 'For Maximum Demand Met in June 2025, do you want the maximum value across all {geographic_level}, or the maximum value for each individual {geographic_level}?'")
            examples.append(f"- 'When you say 'all {geographic_level} in 2025', do you want data for each month of 2025, or the total for the entire year?'")
        
        return "\n".join(examples)

    async def _generate_clarification_question(self, query: str, analysis: QueryAnalysis, request: QueryRequest) -> str:
        """
        ...
        try:
            ...
            clarification_prompt = self._build_contextual_clarification_prompt(
                query, analysis, 
                getattr(request, 'clarification_answers', None)
            )
            ...
            if self.llm_provider:
                ...
                if llm_response and hasattr(llm_response, 'content') and llm_response.content:
                    ...
                    if self._validate_clarification_question(clarification_question):
                        ...
                        logger.warning(f"LLM generated invalid clarification question: {clarification_question}")
    ...
    async def _generate_sql_with_templates(self, query: str, analysis: QueryAnalysis) -> SQLGenerationResult:
        """
        Generate SQL using template-based approach.
        """
        print(f"🔍 RAG TEMPLATE GENERATION CALLED - Query: '{query}', Query Type: {analysis.query_type.value}")
        try:
            logger.info(f"Attempting template-based SQL generation for query: '{query}'")
            logger.info(f"Analysis: type={analysis.query_type}, intent={analysis.intent}")
            ...
            logger.info(f"SQL generation result: success={result.success}, sql_length={len(result.sql) if result.sql else 0}")
    ...
    def _has_sufficient_clarification(self, clarification_answers: Dict[str, str]) -> bool:
        """
        ...
        sufficient_patterns = [
            'energy met', 'total energy', 'maximum energy', 'energy consumption',
            'demand met', 'total demand', 'maximum demand',
            'outage', 'total outage', 'central sector', 'state sector',
            'shortage', 'energy shortage', 'power shortage',
            'generation', 'total generation', 'thermal generation'
        ]
        ...
        for question, answer in clarification_answers.items():
            ...
            for pattern in sufficient_patterns:
                if pattern in answer_lower:
                    logger.info(f"Found sufficient clarification: '{pattern}' in '{answer}'")
                    return True
        
        return False

    def _generate_direct_sql(self, query: str, analysis: QueryAnalysis) -> Optional[str]:
        """
        ...
        try:
            ...
            is_growth_query = any(word in query_lower for word in ['growth', 'increase', 'decrease', 'change', 'trend'])
            is_aggregation_query = any(word in query_lower for word in ['total', 'sum', 'average', 'avg', 'maximum', 'max', 'minimum', 'min', 'aggregate'])
            ...
            if is_growth_query and is_aggregation_query:
                ...
                logger.info(f"Both growth and aggregation detected in direct SQL, prioritizing growth query")
            ...
            if any(word in query_lower for word in ['quarterly', 'quarter', 'q1', 'q2', 'q3', 'q4']):
                ...
                time_period = "daily"
            elif any(word in query_lower for word in ['monthly', 'month', 'by month', 'per month']):
                time_period = "monthly"
            
            logger.info(f"🔍 DIRECT SQL ENHANCED DETECTION - Query: '{query}', Growth: {is_growth_query}, Aggregation: {is_aggregation_query}, Time period: {time_period}")
            
            # For growth queries, redirect to template system for better handling
            if is_growth_query:
                logger.info(f"Growth query detected in direct SQL, redirecting to template system")
            ...
            if hasattr(self, 'schema_linker') and self.schema_linker:
                ...
                if not metric_col:
                    logger.error(f"No column match found for query: {query} in table: {table}. Clarification required.")
                    return None
                
                logger.info(f"Schema linker found column: {metric_col} for table: {table}")
            else:
                logger.error(f"No schema linker available for query: {query}. Clarification required.")
                return None
            
            # Determine the aggregation function
            has_maximum = any(term in query_lower for term in ['maximum', 'max', 'highest', 'top'])
            has_total = any(term in query_lower for term in ['total', 'sum'])
            has_average = any(term in query_lower for term in ['average', 'avg', 'mean'])
    ...
    def _determine_trend_analysis_type(self, query: str, analysis: QueryAnalysis) -> str:
        """
        Determine the type of trend analysis (growth or aggregation) from the query.
        """
        query_lower = query.lower()
        if "growth" in query_lower or "increase" in query_lower or "decrease" in query_lower:
            return "growth"
        elif "maximum" in query_lower or "min" in query_lower or "average" in query_lower or "total" in query_lower:
            return "aggregation"
        else:
...
@lru_cache()
def get_rag_service(db_path: str, llm_provider=None, memory_service=None) -> EnhancedRAGService:
