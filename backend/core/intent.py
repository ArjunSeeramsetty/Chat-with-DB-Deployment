...
class IntentAnalyzer:
    ...
    def __init__(self):
        ...
        self.query_type_keywords = {
            ...
            QueryType.REGION: ['region', 'regions', 'northern', 'southern', 'eastern', 'western'],
            QueryType.GENERATION: ['generation', 'source', 'solar', 'thermal', 'hydro'],
            QueryType.TRANSMISSION: ['transmission', 'line', 'flow', 'import', 'export'],
        ...
        self.intent_keywords = {
            ...
            IntentType.TREND_ANALYSIS: ['trend', 'growth', 'increase', 'decrease', 'over time'],
    ...
    def _detect_query_type(self, query: str) -> QueryType:
        ...
        outage_keywords = ['outage', 'central sector outage', 'state sector outage', 'private sector outage', 'total outage']
        if any(keyword in query_lower for keyword in outage_keywords):
            logger.info(f"Outage keywords detected in query, returning REGION query type for FactAllIndiaDailySummary")
            return QueryType.REGION
        
        # Check for demand keywords - these should also use region-level data for peak demand
        demand_keywords = ['peak demand', 'maximum demand', 'evening peak demand', 'max demand']
        if any(keyword in query_lower for keyword in demand_keywords):
            logger.info(f"Demand keywords detected in query, returning REGION query type for FactAllIndiaDailySummary")
            return QueryType.REGION
        ...
        for state in indian_states:
            if state in query_lower:
                logger.info(f"State '{state}' detected in query, returning STATE query type")
    ...
    def _analyze_intent_patterns(self, query: str) -> IntentType:
        ...
        if any(word in query_lower for word in ['trend', 'over time', 'growth', 'increase', 'decrease']):
            return IntentType.TREND_ANALYSIS
        
        # Check for summary patterns
        if any(word in query_lower for word in ['summary', 'total', 'sum', 'average', 'mean']):
            return IntentType.AGGREGATION
        
        # Default to query
        return IntentType.DATA_RETRIEVAL
    
    async def _analyze_time_period(self, query: str, llm_provider=None) -> Optional[Dict[str, Any]]:
                ...
                if month_name in query_lower:
                    ...
                    for pattern in day_month_year_patterns:
                        ...
                        if match:
                            ...
                            logger.info(f"Found day+month+year pattern: day={day}, month={month_num}, year={year}")
                            result = {
                                'type': 'specific_date',
                                'year': year,
                                'month': month_num,
                    ...
                    if year_match:
                        ...
                        logger.info(f"Found month+year pattern: month={month_num}, year={year}")
                        return {
                            'type': 'specific_date',
                            'year': year,
                            'month': month_num,
            ...
            if date_expressions:
                ...
                if parsed_dates:
                    if len(parsed_dates) == 1:
                        ...
                        is_month_only = bool(re.match(month_year_pattern, original_expr))
...
Return ONLY the JSON object, no explanations or code:"""
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(f"LLM Hook #8: Failed to parse JSON response: {response_content}")
                    return None
    ...
    def _calculate_confidence(self, query: str, query_type: QueryType, intent_type: IntentType) -> float:
        """
    ...
    def _get_detected_keywords(self, query: str) -> List[str]:
        ...
        growth_patterns = [
            'monthly growth', 'quarterly growth', 'yearly growth', 'annual growth',
            'month over month', 'quarter over quarter', 'year over year',
            'growth rate', 'growth percentage', 'growth trend',
            'increase over time', 'decrease over time', 'trend analysis'
        ]
        ...
        time_keywords = ['monthly', 'quarterly', 'yearly', 'annual', 'month', 'quarter', 'year']
