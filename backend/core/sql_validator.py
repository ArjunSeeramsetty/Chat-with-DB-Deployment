...
class SQLValidator:
    ...
    def validate_sql(self, sql: str, dialect: str = "sqlite") -> ValidationResult:
        ...
        if self.schema_info and parse_tree and len(parse_tree) > 0:
            schema_violations = self._validate_schema_compliance(
                parse_tree[0]
            )  # Use first statement
    ...
    def _attempt_sql_fix(self, sql: str) -> Optional[str]:
        ...
        # Fix common issues
        fixed = re.sub(
            r"\bSELECT\s+FROM\b", "SELECT * FROM", fixed, flags=re.IGNORECASE
        )
        fixed = re.sub(
            r"\bGROUP BY\s+ORDER BY\b",
            "GROUP BY 1 ORDER BY",
            fixed,
            flags=re.IGNORECASE,
        )
                ...
                if "." in column_ref:
                    ...
                    if table in self.schema_info:
                        ...
                        if col not in table_columns:
                            violations.append(
                                f"Column '{column_ref}' not found in schema"
                            )
