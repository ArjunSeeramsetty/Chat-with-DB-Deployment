...
class SchemaLinker:
    ...
    def __init__(self, schema_info: Dict[str, List[str]], db_path: str, llm_provider=None):
    ...
    def _get_energy_column_for_table(self, table_name: str, query_type: str = "energy") -> str:
        ...
        if query_type == "evening_demand":
            ...
            transmission_flow_columns = schema_mapping.get('transmission_flow_columns', {})
    ...
    def _get_appropriate_aggregation_functions(self, data_type: str) -> List[str]:
        ...
        aggregation_rules = {
            "power": ["MAX", "MIN", "AVG"],  # Power values - can't sum across time blocks
            "energy": ["SUM", "MAX", "MIN", "AVG"],  # Energy values - can sum
            "ratio": ["AVG", "MAX", "MIN"],  # Ratios - average makes sense
            "percentage": ["AVG", "MAX", "MIN"],  # Percentages (like frequency durations) - average makes sense
            "time": ["SUM", "MAX", "MIN", "AVG"],  # Time durations - can sum
            "outage": ["SUM", "MAX", "MIN", "AVG"],  # Outage values - can sum
            "transmission": ["MAX", "MIN", "AVG"],  # Transmission values (including loading percentages)
    ...
    def _load_dimension_values(self) -> Dict[str, List[str]]:
        ...
        try:
            ...
            for table_name, name_column in dimension_tables.items():
                try:
                    ...
                    if cursor.fetchone():
                        ...
                        cursor.execute(f"SELECT DISTINCT {name_column} FROM {table_name} WHERE {name_column} IS NOT NULL")
                        values = [row[0].lower() for row in cursor.fetchall() if row[0]]
                        dimension_values[table_name] = values
                        logger.info(f"Loaded {len(values)} values from {table_name}: {values[:5]}...")  # Show first 5 values
                    else:
                        logger.warning(f"Table {table_name} not found in database")
                        # Use static values for missing table
                        dimension_values[table_name] = self._get_static_values_for_table(table_name)
                except Exception as e:
            ...
            conn.close()
            logger.info(f"Successfully loaded dimension values for {len(dimension_values)} tables")
    ...
    def _get_static_dimension_values(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            'DimGenerationSources': ['thermal', 'hydro', 'nuclear', 'renewable', 'solar', 'wind', 'biomass'],
    ...
    def _build_schema_embeddings(self):
        ...
        if self.schema_elements:
            ...
            logger.info(f"Built embeddings for {len(self.schema_elements)} schema elements")
        else:
    ...
    def _extract_nl_tokens(self, query: str) -> List[str]:
        ...
        stop_words = {
            ...
            'and', 'or', 'but', 'from', 'by', 'as', 'are', 'were', 'was',
        ...
        meaningful_tokens = [word for word in words if word not in stop_words and len(word) > 2]
    ...
    def _find_schema_links(self, token: str, query_analysis: Any) -> List[SchemaLink]:
        ...
        if hasattr(self, 'schema_vectors') and self.schema_elements:
            try:
                ...
                similarities = cosine_similarity(token_vector, self.schema_vectors).flatten()
        ...
        if hasattr(query_analysis, 'intent'):
            if query_analysis.intent.value == 'trend_analysis':
                ...
                for table, columns in self.schema_info.items():
                    for column in columns:
                        if any(time_word in column.lower() for time_word in ['date', 'time', 'year', 'month', 'quarter']):
                            links.append(SchemaLink(
                                nl_token="trend",
    ...
    def get_enhanced_prompt_context(self, schema_context: SchemaContext) -> str:
        ...
        if schema_context.links:
            ...
            for link in sorted(schema_context.links, key=lambda x: x.confidence, reverse=True)[:10]:
                context_parts.append(f"  '{link.nl_token}' → {link.schema_element} ({link.element_type}, confidence: {link.confidence:.2f})")
        ...
        if schema_context.relevant_columns:
            context_parts.append(f"Relevant Columns: {', '.join(schema_context.relevant_columns)}")
        
        return "\n".join(context_parts) 

    def find_similar_columns(self, user_query: str, table_name: str, top_k: int = 3) -> List[Tuple[str, float]]:
        """
        ...
        try:
            ...
            logger.info(f"Top {top_k} similar columns for query '{user_query}' in table {table_name}:")
    ...
    def _extract_query_words(self, user_query: str) -> List[str]:
        ...
        stop_words = {
            'what', 'is', 'the', 'of', 'in', 'on', 'at', 'to', 'for', 'with', 'by',
            'and', 'or', 'but', 'a', 'an', 'as', 'from', 'into', 'during', 'including',
            'until', 'against', 'among', 'throughout', 'despite', 'towards', 'upon',
            'concerning', 'to', 'in', 'for', 'of', 'with', 'by', 'total', 'sum',
            'maximum', 'minimum', 'average', 'max', 'min', 'avg'
        }
    ...
    def get_best_table_match(self, user_query: str, query_analysis: Any = None) -> Tuple[str, float]:
        """
        ...
        table_patterns = {
            "FactAllIndiaDailySummary": {
                "keywords": ["region", "regional", "all india", "national", "countrywide"],
                "metrics": ["energy", "demand", "shortage", "met", "scada"],
                "priority": 1.0
            },
            "FactStateDailyEnergy": {
                "keywords": ["state", "states", "maharashtra", "karnataka", "tamil nadu", "gujarat"],
                "metrics": ["energy", "demand", "shortage", "met"],
                "priority": 0.9
            },
            "FactDailyGenerationBreakdown": {
                "keywords": ["generation", "power plant", "thermal", "hydro", "renewable", "coal", "gas", "nuclear", "solar", "wind", "biomass"],
                "metrics": ["generation", "output", "capacity", "coal", "gas", "nuclear", "solar", "wind", "biomass"],
                "priority": 0.8
            },
            "FactExchangeData": {
        ...
        for table_name, pattern in table_patterns.items():
            ...
            metric_matches = sum(1 for metric in pattern["metrics"] if metric in query_lower)
            if metric_matches > 0:
            ...
            if any(word in query_lower for word in ["generation", "coal", "gas", "nuclear", "solar", "wind", "biomass", "thermal", "hydro"]):
            ...
            if "state" in query_lower or any(state in query_lower for state in ["maharashtra", "karnataka", "tamil nadu"]):
        ...
        logger.info(f"Auto-selected table '{best_table}' with confidence {best_score:.2f} for query: {user_query[:50]}...")
    ...
    def get_table_config(self, table_name: str, user_query: str) -> Dict[str, str]:
        ...
        if "all" in query_lower and any(word in query_lower for word in ["regions", "states", "countries"]):
            config["where_clause"] = ""  # No filter - include all
        else:
            # Let the assembler handle specific entity filtering
            config["where_clause"] = None
        
        logger.info(f"Auto-configured table '{table_name}': join='{config['join_clause']}', name_col='{config['name_column']}'")
        return config

    def get_best_column_match(self, user_query: str, table_name: str, query_type: str = None, llm_provider=None) -> str:
        """
        ...
        for column in all_columns:
            ...
            if (column_lower in query_lower or 
                any(word in column_lower for word in query_lower.split() if len(word) > 3)):
        ...
        if "energy demand" in query_lower or ("energy" in query_lower and "demand" in query_lower):
        ...
        if "import" in query_lower:
            if "maximum" in query_lower or "max" in query_lower:
                if "MaxImport" in all_columns:
                    logger.info(f"Found 'maximum import' pattern in query, prioritizing MaxImport")
                    return "MaxImport"
            elif "energy" in query_lower or "daily" in query_lower:
                if "ImportEnergy" in all_columns:
                    logger.info(f"Found 'import energy' pattern in query, prioritizing ImportEnergy")
                    return "ImportEnergy"
            elif "net" in query_lower:
                if "NetImportEnergy" in all_columns:
                    logger.info(f"Found 'net import' pattern in query, prioritizing NetImportEnergy")
                    return "NetImportEnergy"
        
        if "export" in query_lower:
            if "maximum" in query_lower or "max" in query_lower:
                if "MaxExport" in all_columns:
                    logger.info(f"Found 'maximum export' pattern in query, prioritizing MaxExport")
                    return "MaxExport"
            elif "energy" in query_lower or "daily" in query_lower:
                if "ExportEnergy" in all_columns:
                    logger.info(f"Found 'export energy' pattern in query, prioritizing ExportEnergy")
                    return "ExportEnergy"
        ...
        if "loading" in query_lower:
            if "maximum" in query_lower or "max" in query_lower:
                if "MaxLoading" in all_columns:
                    logger.info(f"Found 'maximum loading' pattern in query, prioritizing MaxLoading")
                    return "MaxLoading"
            elif "minimum" in query_lower or "min" in query_lower:
                if "MinLoading" in all_columns:
                    logger.info(f"Found 'minimum loading' pattern in query, prioritizing MinLoading")
                    return "MinLoading"
            elif "average" in query_lower or "avg" in query_lower:
                if "AvgLoading" in all_columns:
                    logger.info(f"Found 'average loading' pattern in query, prioritizing AvgLoading")
                    return "AvgLoading"
        ...
        def prioritize_columns(columns):
            ...
            priority_patterns = [
                ...
                lambda col: any(word in col.lower() for word in ['evening', 'peak', 'solar', 'nonsolar']),
                # Then other specific columns (lowest priority)
                lambda col: any(word in col.lower() for word in ['schedule', 'actual', 'overunder', 'net', 'shortage']),
            ...
            for pattern in priority_patterns:
                ...
                remaining_columns = [col for col in remaining_columns if col not in matching]
        ...
        if similar_columns and similar_columns[0][1] > 0.01:  # Lowered threshold for testing
            best_column = similar_columns[0][0]
            logger.info(f"Using similarity-based column match: {best_column} (score: {similar_columns[0][1]:.3f})")
            return best_column
        ...
        if llm_provider and hasattr(llm_provider, 'generate'):
            ...
            try:
                ...
                logger.info(f"LLM Hook #2: Attempting column linking for query: {user_query}")
        ...
        # No fallback - return empty string to indicate no match found
        logger.warning(f"No column match found for query '{user_query}' in table {table_name}")
        return "" 