...
class EnhancedSQLValidator:
        ...
        try:
            ...
            if warnings:
                try:
                    ...
                    logger.warning(f"sqlfluff fix failed: {e}, falling back to manual fixes")
                    fixed_sql = self._manual_style_fix(sql, warnings)
    ...
    def _manual_style_fix(self, sql: str, warnings: List[str]) -> str:
        ...
        for warning in warnings:
            if "L010" in warning:  # Keywords should be consistently upper case
                ...
                keywords = ['SELECT', 'FROM', 'WHERE', 'JOIN', 'ON', 'GROUP BY', 'ORDER BY', 'HAVING', 'LIMIT', 'OFFSET']
                for keyword in keywords:
                    fixed_sql = re.sub(rf'\b{keyword}\b', keyword, fixed_sql, flags=re.IGNORECASE)
                ...
                for line in lines:
                    stripped = line.strip()
                    if stripped:
                        if stripped.upper().startswith(('SELECT', 'FROM', 'WHERE', 'JOIN', 'ON')):
                            indent_level = 0
    ...
    def _validate_security_with_guardrails(self, sql: str) -> Tuple[bool, List[str]]:
        ...
        for pattern in self.dangerous_patterns:
            if re.search(pattern, sql_upper):
                errors.append(f"Security violation: {pattern.strip()} operations are not allowed")
    ...
    def _attempt_auto_repair(self, sql: str, errors: List[str]) -> Tuple[bool, Optional[str]]:
        """
    ...
    async def validate_and_fix_async(self, sql: str, max_attempts: int = 3) -> Tuple[str, ValidationResult]:
        """
